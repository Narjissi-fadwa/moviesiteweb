...
const Section = () => {
    ...
    return (
        <div className="p-6 bg-gray-900 min-h-screen text-white d-flex flex-column  items-center justtify-center w-100">
            ...
            <button onClick={() => { props.setName("jjjjj") }}>ffffff</button>
            ...
            <div className='flex gap-4 items-center justify-center  flex-col p-6'>
                ...
                <input
                    className=" rounded w-full  text-black w-[60vh] py-2 px-5  " type="text" placeholder="Search movie..." value={search}
                    onChange={(e) => setSearch(e.target.value)} />
            </div>

            <div className="grid grid-cols-2 md:grid-cols-4 gap-6  items-center justtify-center" >
                {filteredMovies.map((movie, index) => (
                    <div key={index} onClick={()=>navigate(`/about/${index}/${movie.title}`)} className="bg-gray-800 rounded-lg overflow-hidden shadow-lg w-[20vw] h-[fit-content] ">
